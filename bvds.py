#!/usr/bin/env python3
"""
Bilibili Video Download Script（简称bvds）
"""
...
def add_metadata_to_mp3(mp3_path, source_id, title):
    try:
        temp_path = mp3_path.replace('.mp3', '_temp.mp3')
        comment = f"{source_id}|{title}"
        
        cmd = f'ffmpeg -i "{mp3_path}" -metadata comment="{comment}" -c copy "{temp_path}" -y -hide_banner -loglevel error'
        ...
        if mode == "1":
            ...
            if download_file(session, audio_url, temp, None):
                if check_ffmpeg() and convert_to_mp3(temp, output):
                    add_metadata_to_mp3(output, source_id, clean_title)
                    display.update(task_id, title, "完成")
                    return True
                else:
